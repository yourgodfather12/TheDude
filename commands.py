...
def contains_image_or_video(message):
    return any(attachment.width is not None or attachment.height is not None or attachment.url.endswith(('jpg', 'jpeg', 'png', 'gif', 'webp', 'mp4', 'mov', 'avi')) for attachment in message.attachments)
...
@bot.command(name='cp', help='Checks images or videos posted by a user in the past 5 days and saves them, organized by message content.')
async def check_posts(ctx, member: discord.Member):
    ...
    try:
        ...
        await ctx.send(f'Images or videos posted by {member.mention} in the past 5 days have been saved and data stored successfully.')
